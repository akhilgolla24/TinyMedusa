...
# Adding the timed context manager hook timing function (from the inference notebook)
import time
...
class MedusaModelABC(nn.Module):
    ...
    def medusa_generate(
        ...
        # e.g. top-1 prediction for the next token, top-7 predictions for the next token, top-6 predictions for the next next token.
        medusa_choices=None,
        ...
        sampling = 'typical', 
        fast = True,
        wall_times={'medusa': [], 'tree': [], 'posterior': [], 'update': [], 'init': []}, # For timing Medusa output generation phases
    ):
        ...
        for idx in range(max_steps):
            ...
            with timed(wall_times, 'update'):
                input_ids, logits, medusa_logits, self.new_token_count = update_inference_inputs(
                    input_ids,
                    candidates,
                    best_candidate,
                    accept_length,
